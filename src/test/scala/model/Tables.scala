...
trait Tables extends slicked.SlickMappers {
   ...
   *  @param desc Database column DESC SqlType(VARCHAR), Length(255,true) */
  ...
  class EventTable(_tableTag: Tag) extends profile.api.Table[Event](_tableTag, "EVENT") with SlickedTable {
    ...
    /** Database column DESC SqlType(VARCHAR), Length(255,true) */
    val desc: Rep[Option[String]] = column[Option[String]]("DESC", O.Length(255,varying=true))
  }
