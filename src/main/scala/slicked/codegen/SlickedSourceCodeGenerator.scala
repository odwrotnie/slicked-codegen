...
class SlickedSourceCodeGenerator(model: Model)
  ...
  override def parentType = Some("slicked.SlickMappers")
  ...
  override def tableName =
    (dbTableName: String) => dbTableName.toCamelCase + "Table"
  ...
  override def Table = new Table(_) {
    ...
    override def factory =
      ...
      else s"${EntityType.name}.apply"
    override def extractor =
      if (columns.size <= 22) s"${TableClass.elementType}.unapply"
      else s"${EntityType.name}.unapply"

    override def EntityType = new EntityTypeDef {

      override def parents: Seq[String] =
        Seq("SlickedRow") //, s"${rawName}Api")
      ...
      override def code = {
        ...
        if (classEnabled) {
          ...
          s"""/** Constructor for $name providing default values if available in the database schema. */
             ...
             |    case ${compoundValue(callArgs)} => new $name(${callArgs
               .mkString(", ")})
             |    case _ => throw new Exception("malformed HList")
             |  }
             |  def unapply(row: $name) = Some(${compoundValue(
               callArgs.map(a => s"row.$a"))})
             |}
                """.stripMargin
        }
      }
    ...
    override def PlainSqlMapper = new PlainSqlMapperDef {
      override def code = {
        ...
        val rearranged = compoundValue(desiredColumnOrder.map(i =>
          if (columns.size > 22) s"r($i)" else tuple(i)))
...
//    }
    ...
    override def TableClass = new TableClassDef {
      ...
      override def code = {
        ...
        s"""class $name(_tableTag: Tag) extends profile.api.Table[$elementType](_tableTag, ${args
             .mkString(", ")})$prns {
